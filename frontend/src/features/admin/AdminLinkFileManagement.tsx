    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <OutlinedBtn onClick={() => { void loadFiles(); void loadLogs(); }}>
      ...
      <div>
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          <h2 className="text-lg font-medium" style={{ color: C.onSurface, fontFamily: "Roboto" }}>Access logs</h2>
        ...
        <div className="flex flex-wrap gap-3 items-end mb-3">
          <div className="flex-1 min-w-[180px]">
            <Field label="Search logs" value={logSearch} onChange={setLogSearch} placeholder="Alias, IP, visitor, browser…" />
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm" style={{ fontFamily: "Roboto" }}>
              ...
              <tbody>
                {logsLoading && (
                  <tr>
                    <td colSpan={9} className="p-8 text-center" style={{ color: C.onSurfaceVar }}>Loading…</td>
                  </tr>
                )}
                {!logsLoading && logs.length === 0 && (
                  <tr>
                    <td colSpan={9} className="p-8 text-center" style={{ color: C.onSurfaceVar }}>No access logs</td>
                  </tr>
                )}
                {!logsLoading && logs.map((l) => (
                  <tr key={l.id} style={{ borderTop: `1px solid ${C.outlineVar}` }}>
                    ...
                    <td className="p-3" style={{ color: C.onSurfaceVar }}>{l.ipAddress || "—"}</td>
                    <td className="p-3" style={{ color: C.onSurfaceVar }}>{l.browser || "—"}</td>
                    <td className="p-3" style={{ color: C.onSurfaceVar }}>{l.platform || "—"}</td>
                    <td className="p-3 max-w-[180px] truncate" style={{ color: C.onSurfaceVar }} title={l.referrer || undefined}>
                      {l.referrer || "—"}
                    </td>
                  </tr>
                ))}
